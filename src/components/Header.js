  ...
  return (
    <div className="w-full py-4 flex justify-between absolute bg-gradient-to-b from-black z-10">
        ...
        <div className="flex items-center px-8">
          <button className="text-lg font-bold text-white cursor-pointer bg-red-700 rounded-lg py-2 px-3" onClick={typeof userData !== 'undefined' ? signOutTheUser : signInPage}>{typeof userData !== 'undefined'?'Sign Out' : 'Sign In'}</button>
          </div>
      </div>
  )
}
